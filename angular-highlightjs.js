...
(function (root, factory) {
    ...
    ["hljsCache", "hljsService", "$interpolate", "$window", "$log", function HljsCtrl(hljsCache, hljsService, $interpolate, $window, $log) {
        ...
        ctrl._highlight = function (code, hAttr) {
            if (!_elm) {
                return;
            }
            ...
            if (_interpolateScope) {
                ...
                if (hAttr && hAttr.hljsUnescapeAfter) {
                    // unescape all escaped < and >
                    ...
                    if (found)
                        for (var i = 0; i < found.length; i++)
                            code = code.replace(found[i], found[i].replace(/</g, '&lt;').replace(/>/g, '&gt;'));
                    // and sometimes bad combination of < and / occurs
                    code = code.replace(/><\/</g, '>&lt;/<');
                    code = code.replace(/>></g, '>&gt;<');
                    code = code.replace(/><</g, '>&lt;<');
                }
            ...
            compile: function (tElm, tAttrs, transclude) {
                ...
                return function postLink(scope, iElm, iAttrs, ctrl) {
                    ...
                    if ((staticHTML || staticText) && shouldHighlightStatics(iAttrs)) {
                        ...
                        ctrl.highlight(code, { hljsUnescapeAfter: iAttrs["hljsUnescapeAfter"] });
                    }
            ...
            return {
                ...
                link: function (scope, iElm, iAttrs, ctrl) {
                    ...
                    scope.$watch(iAttrs[dirName], function (newCode, oldCode) {
                        if (newCode) {
                            ctrl.highlight(newCode, { hljsUnescapeAfter: iAttrs["hljsUnescapeAfter"] });
                        }
                        else {
                            ctrl.clear();
                        }
